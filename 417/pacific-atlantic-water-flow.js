...
const pacificAtlantic = function(heights) {
    ...
    const pacificMap = Array(heights.length).fill(false).map(() => Array(heights[0].length).fill(false))
    const atlanticMap = Array(heights.length).fill(false).map(() => Array(heights[0].length).fill(false))
    
    const possibleMoves = [[0, 1], [1, 0], [-1, 0], [0, -1]]
    const bfs = (row, col, map) => {
        ...
        while(bfsQueue.length > 0) {
            ...
            if(!map[node[0]][node[1]]) {
                ...
                for(let move of possibleMoves) {
                    const rowMove = move[0] + node[0]
                    const colMove = move[1] + node[1]
                    if(rowMove >= 0 && rowMove < heights.length && colMove >= 0 && colMove < heights[0].length && heights[node[0]][node[1]] <= heights[rowMove][colMove]) {
                        bfsQueue.push([rowMove, colMove])
                    }
                }
            }
        }
    ...
    for(let row = 0; row < heights.length; row++) {
        bfs(row, 0, pacificMap)
        
        bfs(row, heights[0].length - 1, atlanticMap)
    }
    for(let col = 0; col < heights[0].length; col++) {
        bfs(0, col, pacificMap)
        bfs(heights.length-1, col, atlanticMap)
    }
    
    for(let row = 0; row < heights.length; row++) {
        for(let col = 0; col < heights[0].length; col++) {
            if(pacificMap[row][col] && atlanticMap[row][col]) {
                res.push([row, col])
            }
        }
    }
